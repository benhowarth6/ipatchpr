...
const New = () => {
  ...
  const onSubmit = handleSubmit(async ({ part, device, colour, customer, contactnumber, daterequested, daterequired, paid, ordered }) => {
    ...
    const query = gql`
      mutation CreateATodo($part: String!, $device: String!, $colour: String!, $customer: String!, $contactnumber: String!, $daterequested: Date!, $daterequired: Date!, $paid: Boolean!, $ordered: Boolean!) {
        createTodo(data: { part: $part, device: $device, colour: $colour, customer: $customer, contactnumber: $contactnumber, daterequested: $daterequested, daterequired: $daterequired, paid: $paid, ordered: $ordered }) {
          part
          device
          colour
          customer
    ...
    try {
      await graphQLClient.request(query, { part, device, colour, customer, contactnumber, daterequested, daterequired, paid, ordered });
      Router.push('/');
    } catch (error) {
      console.error(error);
    ...
    <Layout>
       <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold text-gray-900">New Part Request</h1>
        </div>
      </header>
